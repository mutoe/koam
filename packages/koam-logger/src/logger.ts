...
export class Logger {
  ...
  private async writeFileLog(level: LogLevel, ...args: any[]) {
    ...
    if (this.datetimeFormat)
      args.unshift(dateFormat(new Date(), this.datetimeFormat))

    if (!existsSync(this.fileDriver.dir))
      await mkdir(this.fileDriver.dir, { recursive: true })
  ...
  log(...args: any[]): void {
    ...
    if (this.consoleDriver)
      console.log(...args)
    if (this.fileDriver)
      void this.writeFileLog('log', ...args)
  }

  debug(...args: any[]): void {
    if (!this.shouldLogAtLevel('debug'))
      return
    if (this.moduleName)
      args.unshift(`[${this.moduleName}]`)
    args.unshift('[DEBUG]')
    ...
    if (this.consoleDriver)
      console.debug(...args)
    if (this.fileDriver)
      void this.writeFileLog('debug', ...args)
  }

  info(...args: any[]): void {
    if (!this.shouldLogAtLevel('info'))
      return
    if (this.moduleName)
      args.unshift(`[${this.moduleName}]`)
    args.unshift('[INFO]')
    ...
    if (this.consoleDriver)
      console.info(...args)
    if (this.fileDriver)
      void this.writeFileLog('info', ...args)
  }

  warn(...args: any[]): void {
    if (!this.shouldLogAtLevel('warn'))
      return
    if (this.moduleName)
      args.unshift(`[${this.moduleName}]`)
    args.unshift('[WARN]')
    ...
    if (this.consoleDriver)
      console.warn(...args)
    if (this.fileDriver)
      void this.writeFileLog('warn', ...args)
  }

  error(...args: any[]): void {
    if (!this.shouldLogAtLevel('error'))
      return
    if (this.moduleName)
      args.unshift(`[${this.moduleName}]`)
    args.unshift('[ERROR]')
    ...
    if (this.consoleDriver)
      console.error(...args)
    if (this.fileDriver)
      void this.writeFileLog('error', ...args)
  }
}
